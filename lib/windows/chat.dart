...
class _ChatScreenState extends State<ChatScreen> {
  ...
  List<String> _getSortedUsers() {
    ...
    sorted.sort((a, b) {
      ...
      if (!aPinned && bPinned) return 1;
      final aUnread = _unreadCounts.containsKey(a) && (_unreadCounts[a] ?? 0) > 0;
      final bUnread = _unreadCounts.containsKey(b) && (_unreadCounts[b] ?? 0) > 0;
  ...
  Widget _buildMinimizedListItem({
    ...
    return InkWell(
      onTap: onTap,
      child: Container(
        padding: EdgeInsets.symmetric(vertical: _isSidebarMinimized ? 8 : 0),
                ...
                child: Padding(
                  ...
                  child: Row(
                    children: [
                      Expanded(
                        child: Row(
                          children: [
                            ...
                            if (hasDraft) const SizedBox(width: 4),
                            Expanded(
                              child: Text(
                                name,
                      ...
                      if (onPin != null || onAddMember != null)
                        Row(
                          ...
                          children: [
                            ...
                            if (canAddMember && onAddMember != null)
                              IconButton(
      ...
      body: Row(
        children: [
          AnimatedContainer(
            ...
            width: _isSidebarMinimized ? 60 : 280,
            color: const Color.fromARGB(255, 201, 173, 167),
            child: Column(
              children: [
                Container(
                  padding: _isSidebarMinimized
                ...
                ..._getSortedGroups().map(
                  (group) => _buildMinimizedListItem(
                    icon: Icons.group,
                    name: group,
                    isGroup: true,
                    unreadCount: _unreadCounts.containsKey(group) && _unreadCounts[group]! > 0
                        ? _unreadCounts[group]
                        : null,
                    hasDraft: _drafts.containsKey(group) && _drafts[group]!.isNotEmpty,
                    isPinned: _pinnedChats.contains(group),
                    isSelected: _selectedChat == group,
                    canAddMember: group != 'General',
                    onTap: () => _changeChat(group),
                    onPin: () => _togglePin(group),
                    onAddMember: () => _addToGroup(group),
                  ),
                ),
                if (!_isSidebarMinimized)
                  ListTile(
                ...
                Expanded(
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return _buildMinimizedListItem(
                        icon: Icons.person,
                        name: user,
                        isGroup: false,
                        unreadCount: _unreadCounts.containsKey(user) && _unreadCounts[user]! > 0
                            ? _unreadCounts[user]
                            : null,
                        hasDraft: _drafts.containsKey(user) && _drafts[user]!.isNotEmpty,
                        isPinned: _pinnedChats.contains(user),
                        isSelected: _selectedChat == user,
                        onTap: () => _changeChat(user),
                        onPin: () => _togglePin(user),
                      );
                    },
                  ),
                ),
                      ...
                      Expanded(
                        child: Row(
                          children: [
                            ...
                            if (_typingUsers.containsKey(_selectedChat) &&
                                _typingUsers[_selectedChat]!)
                              Padding(
